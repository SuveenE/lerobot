...
Supports two modes:
- **In-process** (default): loads the full VLA model, action head, projectors,
  and runs inference locally.  Requires the ``prismatic`` / ``openvla-oft``
  packages to be installed.
- **External server** (``use_external_server=True``): delegates inference to the
  original openvla-oft ``deploy.py`` FastAPI server over HTTP.  No heavy
  dependencies needed on the lerobot side.
...
class OpenVLAOFTPolicy(PreTrainedPolicy):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        When ``config.use_external_server`` is True, no model is loaded.
        Instead the policy delegates inference to an external openvla-oft
        ...
        # --- External server mode: skip all heavy loading ---
        if config.use_external_server:
            return cls._from_pretrained_external(instance, model_path, config)
        ...
        # 0. For local checkpoints, ensure config.json has the correct auto_map
        #    entries so that AutoModelForVision2Seq resolves to the right classes.
        #    Mirrors the original openvla-oft update_auto_map / check_model_logic_mismatch.
        if is_local:
            instance._sync_auto_map(model_path)
        ...
        #    Skip when use_film=True because _apply_film creates its own
        #    LoRA adapters internally -- applying LoRA here first would
        #    cause a double-application (the merged weights get overwritten
        #    by fresh randomly-initialized LoRA inside _apply_film).
    ...
    @classmethod
    def _from_pretrained_external(
        cls, instance: T, model_path: str, config: OpenVLAOFTConfig
    ) -> T:
        """Lightweight loading path for external-server mode.

        Only loads dataset_statistics.json (if available at model_path) to
        determine action/proprio dims and populate features.  No VLA model,
        action head, or projectors are loaded.
        """
        logger.info(
        ...
        # Verify server is reachable (best-effort, don't fail hard)
        try:
            resp = requests.get(
    ...
    def _predict_action_chunk_external(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # Images: use server_image_key_map to translate lerobot camera keys
        # to the deploy.py observation dict keys.
        ...
        # Proprio state (raw, unnormalized -- the server normalizes internally)
        ...
        # action_list is either a list of numpy arrays (one per timestep)
        # or a single numpy array of shape (chunk_size, action_dim)
    ...
    @staticmethod
    def _sync_auto_map(checkpoint_dir: str) -> None:
        """Ensure the local checkpoint's config.json has the correct auto_map entries.

        Mirrors the original openvla-oft ``update_auto_map`` so that
        ``AutoModelForVision2Seq.from_pretrained`` resolves to
        ...
        if cfg_data.get("auto_map") == expected_auto_map:
            return

        logger.info(f"Updating auto_map in {config_path}")
        cfg_data["auto_map"] = expected_auto_map
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], **kwargs) -> Tensor:
        ...
        Dispatches to the external HTTP server when ``use_external_server``
        is True, otherwise runs the full VLA pipeline in-process.
        """
        if self.config.use_external_server:
            return self._predict_action_chunk_external(batch)
