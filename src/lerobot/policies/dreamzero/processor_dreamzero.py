"""Minimal pre/post processors for DreamZero.

DreamZero handles its own image preprocessing (resize, frame accumulation)
and action denormalization on the external server. These processors only handle:
...
def make_dreamzero_pre_post_processors(
    ...
    """Create minimal pre/post processor pipelines for DreamZero.

    No normalization or unnormalization is applied since the DreamZero
    server handles these internally.
    """
