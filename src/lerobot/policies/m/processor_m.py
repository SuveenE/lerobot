"""Minimal pre/post processors for the M policy.

The external server handles its own image preprocessing (resize, normalize)
and action (un)normalization internally. These processors only handle:
...
def make_m_pre_post_processors(
    config: MConfig,
    ...
    """Create minimal pre/post processor pipelines for the M policy.

    No normalization or unnormalization is applied since the external server
