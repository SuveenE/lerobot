...
def require_package(pkg_name: str, extra: str = ""):
    """Check that a package is available, raise ImportError if not."""
    if not _transformers_available and pkg_name == "transformers":
        msg = f"'{pkg_name}' is required but not installed."
        if extra:
            msg += f" Install with: pip install -e .[{extra}]"
