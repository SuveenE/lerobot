"""Minimal pre/post processors for Cosmos Policy.

Cosmos Policy's external ``deploy.py`` handles its own image preprocessing
(resize to 224x224, RGB float conversion), proprio normalization, and action
unnormalization using the ``dataset_statistics.json`` passed at server launch.
These processors therefore only handle:

- Preprocessor: optional key renaming, batch dimension, device placement
...
def make_cosmos_pre_post_processors(
    ...
    """Create minimal pre/post processor pipelines for Cosmos Policy.

    No normalization or unnormalization is applied; ``deploy.py`` does that
    server-side using its own ``dataset_statistics.json``.
    """
