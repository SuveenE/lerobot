#!/usr/bin/env python3
"""
GELLO arm server that exposes joint positions and encoder data for teleoperation.

This script wraps the i2rt robot to expose GELLO leader arm state through
the portal RPC interface, allowing LeRobot to read joint positions and 
gripper commands from the GELLO device.

Based on yam_server_with_encoder.py but adapted for GELLO hardware.
"""
...
class EnhancedGelloRobot(Robot):
    """
    Wrapper around MotorChainRobot that exposes encoder data for GELLO leaders.
    Reads encoder position and button states to provide gripper control information.
    """

    def __init__(self, robot: MotorChainRobot, has_encoder: bool = True):
        self._robot = robot
        self._motor_chain = robot.motor_chain
        self._has_encoder = has_encoder
    ...
    def get_joint_pos(self) -> np.ndarray:
        ...
        # For GELLO with encoder, add gripper state from encoder
    ...
    def command_joint_pos(self, joint_pos: np.ndarray) -> None:
        ...
        # For GELLO leaders, ignore gripper command if included (passive device)
        if self._has_encoder and len(joint_pos) > self._robot.num_dofs():
    ...
    def get_observations(self) -> Dict[str, np.ndarray]:
        ...
        For GELLO with encoder, includes:
        ...
        # For GELLO with encoder, add encoder data
        if self._has_encoder:
            try:
                encoder_states = self._motor_chain.get_same_bus_device_states()
                if encoder_states and len(encoder_states) > 0:
...
class ServerRobot:
    ...
    def __init__(self, robot: Robot, port: int):
        ...
        print(f"GELLO Robot Server Binding to {port}, Robot: {robot}")
...
class Args:
    ...
    can_channel: str = "can_gello_r"
    has_encoder: bool = True


def main(args: Args) -> None:
    """Main function to start the GELLO server."""
    ...
    robot = EnhancedGelloRobot(base_robot, has_encoder=args.has_encoder)

    # Start the server
    server_robot = ServerRobot(robot, args.server_port)
    print(f"\n{'='*60}")
    print("GELLO Server Started")
    print(f"  CAN Channel: {args.can_channel}")
    print(f"  Gripper Type: {args.gripper}")
    print(f"  Has Encoder: {args.has_encoder}")
    print(f"  Port: {args.server_port}")
    if args.has_encoder:
        print("  Encoder Support: ENABLED ✓")
    print(f"{'='*60}\n")
