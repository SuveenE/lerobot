...
- Enable/disable torque programmatically (`enable_torque` / `disable_torque`)
- Move to target positions (`write_goal_positions`) to mirror the robot state when pausing

**Compatible teleoperator in this fork:**

- `bi_yam_leader` - Bimanual YAM leader arms. Each leader is a full 6-DoF
  `MotorChainRobot` (DM4310 motors, same class as the follower) plus a passive
  teaching-handle gripper. In normal teleop the arms are gravity-compensated
  free-drive (`kp≈0`); during the pre-takeover sync the script raises `kp` via
  `BiYamLeader.enable_torque()` and drives the leader to the follower's current
  pose using `write_goal_positions(...)`.

#### YAM leader active-tracking tunables

`BiYamLeaderConfig` exposes two tunables used only during the 2 s pre-sync:

- `active_kp: float = 30.0` — position gain applied to all 6 arm joints.
- `active_kd: float = 1.0` — velocity gain applied to all 6 arm joints.

These defaults are starting points. If the leader lags behind the follower
during the sync move, raise `active_kp` slightly; if it jolts or oscillates,
reduce `active_kp` (and/or raise `active_kd`). Values are reset to zero by
`disable_torque()` before control is handed back to the human.

> [!IMPORTANT]
> The YAM leader servers must be restarted after upgrading so the enhanced
> `yam_server_with_encoder.py` binds the new `update_kp_kd` RPC in addition to
> `command_joint_pos` / `command_joint_state`. Restart via
> `setup_bi_yam_servers.py` (ports 5001 / 5002 by default).

> [!NOTE]
> The teaching-handle gripper on each YAM leader is a passive encoder + button
> and cannot be commanded. `BiYamLeader.write_goal_positions(...)` silently
> drops any `*gripper*` keys for this reason; the 6 arm joints are the only
> commandable degrees of freedom.
...
The YAM leader servers (ports 5001 / 5002) must be running beforehand — see
