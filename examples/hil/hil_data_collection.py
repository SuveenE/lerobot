...
Usage (bi_yam follower + bi_yam leader):
    ...
    # With RTC for large models (Pi0, Pi0.5, SmolVLA, OpenVLA-OFT)
    python examples/hil/hil_data_collection.py \
        --rtc.enabled=true \
...
# Register bi_yam configs so draccus resolves --robot.type=bi_yam_follower and
# --teleop.type=bi_yam_leader at CLI parse time.
from lerobot.robots import Robot, RobotConfig, make_robot_from_config
from lerobot.robots.bi_yam_follower.config_bi_yam_follower import BiYamFollowerConfig  # noqa: F401
from lerobot.teleoperators import Teleoperator, TeleoperatorConfig, make_teleoperator_from_config
from lerobot.teleoperators.bi_yam_leader.config_bi_yam_leader import BiYamLeaderConfig  # noqa: F401
...
# Relative-action RTC re-anchoring was added upstream alongside a
# RelativeActionsProcessorStep / to_relative_actions pair that does not exist in
# this fork. We leave the plumbing in place but treat relative_step as always
# None, which effectively disables re-anchoring and falls back to absolute RTC
# prefixes — the standard behavior for absolute-action policies.
RelativeActionsProcessorStep = None  # type: ignore[assignment]


def to_relative_actions(*args, **kwargs):  # type: ignore[no-redef]
    raise RuntimeError(
        "to_relative_actions is not available in this fork. "
        "Relative-action RTC re-anchoring is disabled; use an absolute-action policy."
    )
...
def _rtc_inference_thread(
    ...
    relative_step = (
        ...
        if RelativeActionsProcessorStep is not None
        else None
    )
