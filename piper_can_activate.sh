...
# If $1 is omitted: single-adapter runs rename the interface to this name. For bimanual setups,
# pass explicit names (e.g. right_piper_can / left_piper_can), as record.sh does.
DEFAULT_CAN_NAME="${1:-left_piper_can}"
...
if [ "$CURRENT_CAN_COUNT" -ne "1" ]; then
    if [ -z "$USB_ADDRESS" ]; then
        ...
        echo -e " bash piper_can_activate.sh right_piper_can 1000000 1-2:1.0"
        echo "-------------------ERROR-----------------------"
        exit 1
    fi
fi
...
if [ "$IS_LINK_UP" == "yes" ] && [ "$CURRENT_BITRATE" -eq "$DEFAULT_BITRATE" ]; then
    ...
    if [ "$INTERFACE_NAME" != "$DEFAULT_CAN_NAME" ]; then
        ...
        # If the target name is occupied by another interface, swap via a temp name
        if ip link show "$DEFAULT_CAN_NAME" &>/dev/null; then
            TEMP_NAME="can_tmp_$$"
            echo "目标名称 $DEFAULT_CAN_NAME 已被占用，通过临时名称 $TEMP_NAME 进行交换"
            sudo ip link set "$DEFAULT_CAN_NAME" down
            sudo ip link set "$DEFAULT_CAN_NAME" name "$TEMP_NAME"
            sudo ip link set "$INTERFACE_NAME" name "$DEFAULT_CAN_NAME"
            sudo ip link set "$TEMP_NAME" name "$INTERFACE_NAME"
